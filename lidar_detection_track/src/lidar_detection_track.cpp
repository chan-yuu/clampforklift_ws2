...
lidarPerception::lidarPerception(ros::NodeHandle nh, ros::NodeHandle pnh)
	: roiClip_(nh, pnh), voxelGridFilter_(nh, pnh), cluster_(nh, pnh), boundingBox_(pnh), tf_listener(tf_buffer)
{
	// nuscenes的 topic
	// ros::Subscriber sub = nh.subscribe("/LIDAR_TOP", 1, &lidarPerception::ClusterCallback, this);
	ros::Subscriber sub = nh.subscribe("/rslidar_points", 1, &lidarPerception::ClusterCallback, this);

	_pub_clip_cloud = nh.advertise<sensor_msgs::PointCloud2>("/xh/points_clip", 10);
	_pub_in_cloud = nh.advertise<sensor_msgs::PointCloud2>("/xh/points_in", 10);
	_pub_cluster_cloud = nh.advertise<sensor_msgs::PointCloud2>("/points_cluster", 10);
	_pub_noground_cloud = nh.advertise<sensor_msgs::PointCloud2>("/xh/nopoints_ground", 10);
	_pub_ground_cloud = nh.advertise<sensor_msgs::PointCloud2>("/xh/points_ground", 10);

	_pub_clusters_message = nh.advertise<autoware_msgs::CloudClusterArray>("/detection/lidar_detector/cloud_clusters", 10);
	_pub_detected_objects = nh.advertise<autoware_msgs::DetectedObjectArray>("/detection/lidar_detector/objects", 10);
	_pub_cluster_visualize_markers = nh.advertise<visualization_msgs::MarkerArray>("/xh/visualize/cluster_markers", 10);
	...
	const sensor_msgs::PointCloud2ConstPtr &in_sensor_cloud)
{
    ...
    transformStamped = tf_buffer.lookupTransform("base_footprint", "rslidar", ros::Time(0));
    // 将变换转换为Eigen矩阵
    Eigen::Affine3d base_to_sensor_affine = tf2::transformToEigen(transformStamped);
    base_to_sensor_matrix = base_to_sensor_affine.matrix().cast<float>();
    ...
    roiClip_.GetROI(in_cloud_ptr, clip_cloud_ptr, base_to_sensor_matrix);
