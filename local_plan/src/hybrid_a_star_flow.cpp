...
HybridAStarFlow::HybridAStarFlow(ros::NodeHandle &nh) {
    ...
    // 局部化处理：
    global_path_sub_ = nh.subscribe("/global_path", 1, &HybridAStarFlow::globalPathcallback, this); // 同上
    costmap_sub_ptr_ = std::make_shared<CostMapSubscriber>(nh, "/local_map", 1);
    ...
    // 规划状态机主函数：
    timer_ = nh.createTimer(ros::Duration(0.5), &HybridAStarFlow::timerCallback, this);
...
void HybridAStarFlow::globalPathcallback(const nav_msgs::Path::ConstPtr& msg) 
{
    // std::lock_guarcd<std::mutex> lock(data_mutex_);
    // std::cout << "global path callback" << std::endl;
...
void HybridAStarFlow::timerCallback(const ros::TimerEvent&) 
{
    ...
    {
        // TODO 10ms的规划
        if (!have_odom_ || global_path_.poses.size()<1)
        {
            ROS_INFO("\033[1;31m Wait Odom & Path ");
            std::cout << "have_odom_" <<have_odom_ << "global_path_" << global_path_.poses.size() << std::endl;
            return;
        }
        costmap_sub_ptr_->ParseData(costmap_deque_);
        if (!has_map_){
            ...
            std::cout << "x_lower: " << current_costmap_ptr_->info.origin.position.x << std::endl;
            std::cout << "x_upper: " << 1.0 * current_costmap_ptr_->info.width * current_costmap_ptr_->info.resolution << std::endl;
            std::cout << "y_lower: " << current_costmap_ptr_->info.origin.position.y << std::endl;
            std::cout << "y_upper: " << 1.0 * current_costmap_ptr_->info.height * current_costmap_ptr_->info.resolution << std::endl;
            std::cout << "state_grid_resolution: " << 1.0 << std::endl;
        ...
        double pallet_heading = 0.0;
        double pallet_x = 0.0;
        double pallet_y = 0.0;
        double roll, pitch;

        if (global_path_.poses.size() >= path_index_dis + 50) {
            geometry_msgs::PoseStamped pose_30 = global_path_.poses[path_index_dis + 49];
            pallet_x = pose_30.pose.position.x;
            pallet_y = pose_30.pose.position.y;

            tf::Quaternion q(
                pose_30.pose.orientation.x,
                pose_30.pose.orientation.y,
                pose_30.pose.orientation.z,
                pose_30.pose.orientation.w);
            tf::Matrix3x3(q).getRPY(roll, pitch, pallet_heading);
        } else {
            geometry_msgs::PoseStamped pose_30 = global_path_.poses[global_path_.poses.size()-1];
            pallet_x = pose_30.pose.position.x;
            pallet_y = pose_30.pose.position.y;
        ...
        Vec3d start_state = Vec3d(current_position.x, current_position.y, current_position.yaw);
        Vec3d goal_state = Vec3d(pallet_x, pallet_y, pallet_heading);
        
        // 打印起点信息
        ROS_INFO_STREAM("\033[1;33m Start Pose [" << current_position.x << ", " << current_position.y << ", " << current_position.yaw << ") rad");
        ROS_INFO_STREAM("\033[1;33m Goal Pose [" << pallet_x << ", " << pallet_y << ", " << pallet_heading << ") rad");
...
void HybridAStarFlow::odometryCallback(const nav_msgs::Odometry::ConstPtr& msg) 
{
    ...
    if (global_path_.poses.size()>1){
        size_t closest_index_dis = findClosestPathPointIndex(global_path_, current_position);
        path_index_dis = closest_index_dis;
    }
}

// 已经找到map的最近索引之后，查找指定路径中最近的路径点的索引 
size_t HybridAStarFlow::findClosestPathPointIndex(const nav_msgs::Path& path, const Point& odom_pos_) 
{
    ...
    for (size_t path_index = 0; path_index < path.poses.size(); ++path_index) {
        // 获取路径点的位置
        double path_x = path.poses[path_index].pose.position.x;
        double path_y = path.poses[path_index].pose.position.y;
        double path_yaw = 0.0; // 不需要考虑yaw角
        Point current_point = {path_x, path_y, path_yaw};
...
// 计算两点之间的欧氏距离
double HybridAStarFlow::calculateDistance(const Point& p1, const Point& p2) 
{
