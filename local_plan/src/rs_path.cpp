...
//函数的目的是将RS曲线转换为实际的路径点，这些点可以用于机器人或车辆的导航。
//通过插值，它可以生成任意分辨率的路径点，而转弯半径允许调整路径的弯曲程度，以适应不同的规划场景。
