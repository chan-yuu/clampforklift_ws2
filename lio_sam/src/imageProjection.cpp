...
private:
    ...
    pcl::PointCloud<RSlidarPointXYZIRT>::Ptr tmpRslidarCloudIn;
...
public:
    ...
    deskewFlag(0)
    {
        ...
        // save_path = nh.subscribe<nav_msgs::Odometry>("lio_sam/mapping/odometry",   100, &ImageProjection::path_save_handler, this, ros::TransportHints().tcpNoDelay()); 
    ...
    // void path_save_handler(nav_msgs::Odometry odomAftMapped ){
    ...
    void allocateMemory()
    {
        ...
        tmpRslidarCloudIn.reset(new pcl::PointCloud<RSlidarPointXYZIRT>());
        ...
        bool cachePointCloud(const sensor_msgs::PointCloud2ConstPtr& laserCloudMsg)
    {
        ...
        if (sensor == SensorType::VELODYNE)
        {
            ...
            std::cout<<"time:"<<laserCloudIn->points[0].time<<std::endl;
        ...
        else if (sensor == SensorType::RSLIDAR)
        {
            ...
            pcl::moveFromROSMsg(currentCloudMsg, *tmpRslidarCloudIn);
             std::vector<int> indices;
            pcl::removeNaNFromPointCloud(*tmpRslidarCloudIn, *tmpRslidarCloudIn, indices);
            
            
             double start_timestamp = tmpRslidarCloudIn->points[0].timestamp;
            ...
            for (size_t i = 0; i < tmpRslidarCloudIn->size(); i++)
            {
                ...
                dst.intensity = src.intensity;
                dst.ring = src.ring;
                dst.time = static_cast<float>(src.timestamp-start_timestamp);
    ...
    bool deskewInfo()
    {
        ...
        std::cout<<"imuQueue.back().header.stamp.toSec():"<<imuQueue.back().header.stamp.toSec()<<std::endl;
        std::cout<<"timeScanEnd:"<<timeScanEnd<<std::endl;
        std::cout<<imuQueue.back().header.stamp.toSec()-timeScanEnd<<std::endl;
   ...
   void projectPointCloud()
{
    ...
    for (int i = 0; i < cloudSize; ++i)
    {
        ...
        if (rangeMat.at<float>(rowIdn, columnIdn) != FLT_MAX)
            continue;
        // 如果使用ring信息，对点进行去畸变处理
        if (has_ring == true)
            thisPoint = deskewPoint(&thisPoint, laserCloudIn->points[i].time);
            
        else {
                // 如果没有ring信息，对点进行去畸变处理，并根据半圈判断调整时间戳
