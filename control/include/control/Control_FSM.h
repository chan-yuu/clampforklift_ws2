...
struct SpeedParams{
  double num_smaller_path_speed_max;
  double num_smaller_path_speed_min;
  double num_normal_path_speed_max;
  double num_normal_path_speed_min;
  double speed_very_high;
  double speed_high;
  double speed_medium;
  double speed_medium_medium;
  double speed_low;
  double speed_very_low;
};
...
private:
  ...
  // 定位：
  Point current_position, front_middle_position, front_current_position;
  // 计算终点：
  Point dis_current_position;
  double current_path_curvature, current_path_curvature_modify;
  ...
  size_t path_index_front;
  double kp_front, kp_normal;
  ...
  // 输出结果
  //outWheelAngle CalController(int nearest_index, const CustomPath& path, Point loc_pose, double curvature,double current_curvature, double current_steering_);
  //outWheelAngle CalControllerREVERSE(int nearest_index, const CustomPath& path, Point loc_pose, double curvature,double current_curvature, double current_steering_);
  
  outWheelAngle CalController_TURN_speed(int nearest_index, const CustomPath& path, Point loc_pose, double curvature,double current_curvature, double current_steering_);
  
  //outWheelAngle CalControllerLocal(int nearest_index, const CustomPath& path, Point loc_pose, double curvature,double current_curvature, double current_steering_);
  //outWheelAngle CalControllerREVERSELocal(int nearest_index, const CustomPath& path, Point loc_pose, double curvature,double current_curvature, double current_steering_);

  outWheelAngle CalController(int closest_index, int closest_index_modify, double curvature, double pre_curvature, const CustomPath& path, Point loc_pose);
  outWheelAngle CalControllerREVERSE(int closest_index, int closest_index_modify, double curvature, double pre_curvature, const CustomPath& path, Point loc_pose);
  outWheelAngle CalControllerLocal(int closest_index, int closest_index_modify, double curvature, double pre_curvature, const CustomPath& path, Point loc_pose);
  outWheelAngle CalControllerREVERSELocal(int closest_index, int closest_index_modify, double curvature, double pre_curvature, const CustomPath& path, Point loc_pose);

  outWheelAngle Calkappagear(int nearest_index, const CustomPath& path);
  ...
  //速度函数：
  double GetLowerSpeed(double cte, double dhead, double wheel_angle_rad);
  // void GetSpeed(std::vector<double>& curvature_list,int i,std::vector<double>& vec);
  std::vector<double> GetSpeed(const std::vector<double>& curvature_list);

  Point movePointAlongHeading(const Point& currentPoint, double distance);
  bool exec_kappa;
  Point previousPoint;
  bool firstTime = true;
  bool firstTime_reverse = false;
  double totalDistance = 0.0;
  double Reverse_distance;
  int findNextAvailablePathNumber(const std::string& folderPath);
...
//   MPCController::Ptr mpccontroller_;
 std::unique_ptr<StanleyController> stanleycontroller_;  
  std::unique_ptr<PDController> pdcontroller_;  
  ...
  int pre_index_, pre_index_front, pre_index_normal;  //档位育苗点，用于判断匹配点和育苗点 之间是否需要换档
  double yawerror;
  double cte;
  ...
  ros::Subscriber path_sub_, odom_sub_ ,stop_sub_,gear_sub_,locale_sub_, odom_front_front_sub_;
  ...
  ros::Subscriber odom_front_sub_, odom_front_sub_fix;
  
  ros::Subscriber steering_state_sub_;
  ros::Publisher updown_cmd_pub, clamp_cmd_pub,lateral_cmd_pub,fy_cmd_pub ;
  std_msgs::Float64 updown_msg;
  std_msgs::Float64 clamp_msg, lateral_msg, fy_msg;
  ...
  void frontodometryCallback(const nav_msgs::Odometry::ConstPtr& msg);
  ...
  Point interpolatePoints(const Point& start, const Point& end, double k);
  Point current_position_modify;

  void publishOdom(const Point& position);
  ros::Publisher odom_pub;

  double Rotation_angle;
  double Angle_Change;
  double yawNeedchange;
  double Throttle_Rotation;
  double Steering_compensation;
  int prevPathLength = 0;
  std::pair<int, double> calculateOptimalRotation(double currentYaw, double goalYaw);
  double normalizeAngle(double angle);

  SpeedParams speed_params_;
public:
  ControlFSM(/* args */) {
  }
  ~ControlFSM() {
