...
class GlobalPickupTask_1(smach.State, SerManager):
    def __init__(self,limited_speed=None):
        smach.State.__init__(self, outcomes=['finish'], io_keys=['conveyer_belt_point', 'observer_point', 'delivery_point'])
        SerManager.__init__(self)
    ...
    def execute(self, userdata):
        ...
        self.send_plan_task_service(self.task)
        self.send_control_task_service(self.task, self.limited_speed)
        # self.send_stop_task_service(self.task) # 0-不响应 1-响应停障
        rospy.loginfo("\033[32m"+"任务下发成功"+"\033[0m")
...
class GlobalPickupTask_2(smach.State, SerManager):
    ...
    def execute(self, userdata):
        ...
        reversed_goal_yaw_point = self.calculate_reverse_point(point_observe, 2.5+cotton_4_num*0.85) # 后移 要求检测距离为3.5m时。
...
class GlobalPickupTask_3(smach.State, SerManager):
    ...
    def execute(self, userdata):
        ...
        point_observe = self.calculate_new_point(current_pos_point, 2, 0.45) # 观察点不动 2-左
        back_point_observe = self.calculate_reverse_point(point_observe, 1)
        back_back_point_observe = self.calculate_reverse_point(point_observe, 3)

        self.new_point_reindex.append(back_point_observe)
        ...
        rospy.loginfo("\033[32m"+"下发目标点位"+"\033[0m")
        goal_point, goal_quaternion = back_back_point_observe
        goal_pose = Point(goal_point[0], goal_point[1], goal_point[2])
        goal_orientation = Quaternion(goal_quaternion[0], goal_quaternion[1], goal_quaternion[2], goal_quaternion[3])
        self.send_target_pose(goal_pose, goal_orientation)
        rospy.loginfo("\033[32m"+"目标点位发布成功"+"\033[0m")
    ...
    with sm:
        ...
        with sub_sm:
            ...
            with pickup_sub_sm:
                ...
                smach.StateMachine.add('GLOBAL_PICKUP_1', GlobalPickupTask_1(limited_speed=0.5), transitions={'finish': 'SET_WIDTH_PICKUP'})
                if not use_sim:
                    # 取一个包
            ...
            with parallel_container3:
                smach.Concurrence.add('GLOBAL_MOVE_BACK', GlobalMoveTask_OpenLoop(target_distance=(-2.3), Throttle=0.5))
                if not use_sim:
            ...
            with parallel_container4:
                smach.Concurrence.add('GLOBAL_PICKUP_2', GlobalPickupTask_1(limited_speed=0.5))
                if not use_sim:
            ...
            smach.StateMachine.add('GLOBAL_PICKUP_3', GlobalPickupTask_2(limited_speed=0.8), transitions={'finish': 'ADJUST_HEADING'})

            if not use_sim:
                # 调整航向
                smach.StateMachine.add('ADJUST_HEADING', AdjustHeading(goal='observer_point', steering_angle=adjust_steering_angle, changeYawangle=-90, gear=3, throttle=0.5, control_com=8),
                                        # 都是角度制
                                        transitions={'finish': 'GLOBAL_PICKUP_FINAL'})
                smach.StateMachine.add('ADJUST_DESIGN_HEADING', AdjustHeading_Design_Angle(angle=90,steering_angle=adjust_steering_angle, gear=3, throttle=0.4, control_com=6),
                                        # 都是角度制
                                        transitions={'finish': 'GLOBAL_PICKUP_FINAL'})
            else:
                # 调整航向
                smach.StateMachine.add('ADJUST_HEADING', AdjustHeading(goal='observer_point', steering_angle=adjust_steering_angle_sim, changeYawangle=-90, gear=3, throttle=0.4, control_com=0),
                                        # 都是角度制
                                        transitions={'finish': 'GLOBAL_PICKUP_FINAL'})
                smach.StateMachine.add('ADJUST_DESIGN_HEADING', AdjustHeading_Design_Angle(angle=90,steering_angle=adjust_steering_angle_sim, gear=3, throttle=0.4, control_com=0),transitions={'finish': 'GLOBAL_PICKUP_FINAL'})
            # 到位
            smach.StateMachine.add('GLOBAL_PICKUP_FINAL', GlobalMoveTask_CloseLoop(target_distance=1.6, limited_speed=0.8), transitions={'finish': 'FINISH'})
            # 张开
            smach.StateMachine.add('WIDTH_OPEN_FINAL', SetWidth(clamping_width), transitions={'finish': 'finish'})
            
            smach.StateMachine.add('FINISH', Finish(), transitions={'succeed':'finish'})

        smach.StateMachine.add('SUB_STATE_MACHINE', sub_sm, transitions={'finish': 'GLOBAL_PICKUP_FINAL1'})
        # 闭环倒车
        smach.StateMachine.add('GLOBAL_PICKUP_FINAL1', GlobalMoveTask_CloseLoop(target_distance=-1.6, limited_speed=0.8), transitions={'finish': 'ADJUST_HEADING1'})
        
        # 调整航向
        smach.StateMachine.add('ADJUST_HEADING1', AdjustHeading(goal='observer_point', steering_angle=adjust_steering_angle, changeYawangle=0, gear=1, throttle=0.5, control_com=8),transitions={'finish': 'GLOBAL_PICKUP_FINAL2'})
        # 闭环前进一点
        smach.StateMachine.add('GLOBAL_PICKUP_FINAL2', GlobalMoveTask_CloseLoop(target_distance=2.2, limited_speed=0.8), transitions={'finish': 'SUB_STATE_MACHINE'})
        
        # 第二次作业过程
        smach.StateMachine.add('SUB_STATE_MACHINE1', sub_sm, transitions={'finish': 'FOUR'})

        # 嵌套状态机开始
        final_pickup_sub_sm = smach.StateMachine(outcomes=['finish'])
        with final_pickup_sub_sm:
            # 闭环倒车
            smach.StateMachine.add('GLOBAL_PICKUP_FINAL3', GlobalMoveTask_CloseLoop(target_distance=-1.2, limited_speed=0.8), transitions={'finish': 'GLOBAL_PICKUP_FINAL4'})
            # 再倒车
            smach.StateMachine.add('GLOBAL_PICKUP_FINAL4', GlobalPickupTask_3(limited_speed=0.8), transitions={'finish': 'WIDTH_OPEN_SECOND'})

            # 张开
            smach.StateMachine.add('WIDTH_OPEN_SECOND', SetWidth(clamping_width), transitions={'finish': 'HEIGHT_RAISE_SECOND'})
            # 高度
            smach.StateMachine.add('HEIGHT_RAISE_SECOND', SetHeight(height_between_two_cargos+5, updown_mode=2),transitions={'finish': 'CAMERA_TASK_3'})

            # 检测四包
            smach.StateMachine.add('CAMERA_TASK_3', CAMERA_TASK(goal=4, use_camera_det=use_camera_det), transitions={'finish': 'GLOBAL_PICKUP_4'})
            # 夹取四包
            smach.StateMachine.add('GLOBAL_PICKUP_4', GlobalPickupTask_1(limited_speed=0.5), transitions={'finish': 'WIDTH_OPEN_THIRD'})

            # 夹紧
            smach.StateMachine.add('WIDTH_OPEN_THIRD', SetWidth(clamping_width), transitions={'finish': 'HEIGHT_RAISE_THIRD'})
            # 高度
            smach.StateMachine.add('HEIGHT_RAISE_THIRD', SetHeight(height_between_two_cargos+5, updown_mode=2),transitions={'finish': 'finish'})
        smach.StateMachine.add('FOUR', final_pickup_sub_sm, transitions={'finish':'FINISH'})
