...
Date: 2024-10-08 01:49:52
LastEditors: CYUN && cyun@tju.enu.cn
LastEditTime: 2025-01-08 14:56:48
FilePath: /undefined/home/cyun/forklift_sim_ws3/src/smach_fork/scripts/smach_viewer.py
Description: 通过samch可视化整体调度任务

Copyright (c) 2024 by Tianjin University, All Rights Reserved. 
'''
import os
os.environ['GDK_DISABLE_SETLOCALE'] = '1'
import logging
...
logging.getLogger().setLevel(logging.WARNING)
# 忽略没有订阅者的警告
rospy.ROS_WARN_PARAMODAL = False
...
class SmachViewerFrame(wx.Frame, SmachViewerBase):
    ...
    #         rospy.loginfo_once("Publishing {} requires at least one subscriber".format(self._pub.name))
    #         return
    #     # image
    ...
    def SaveDotGraph(self, event):
        ...
        try:
            ...
            directory = os.path.join(package_path, 'dot')
            if not os.path.exists(directory):
                os.makedirs(directory)
            filename = os.path.join(directory, timestr + '.dot')
            print('Writing to file: %s' % filename)
            with open(filename, 'w') as f:
                f.write(self.dotstr)
        except rospkg.common.ResourceNotFound as e:
            print(f"Error: Package 'smach_fork' not found. {e}")
...
if __name__ == '__main__':
    rospy.init_node('smach_viewer',anonymous=True, log_level=rospy.INFO)
