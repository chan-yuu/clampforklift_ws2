...
if not os.path.exists(points_file_path):
    with open(points_file_path, 'w') as txtfile:
        txtfile.write("# Saved Points\n")
...
class LidarObstacle(smach.State, SerManager):
    ...
    def execute(self, userdata):
        ...
        point_pose5 = self.calculate_reverse_point(point_pose4, -16)
        ...
        self.new_point_reindex.append(delivery_point7)
        # self.new_point_reindex.append(delivery_point6)
        ...
        goal_point, goal_quaternion = delivery_point6
...
class GlobalDelivery(smach.State, SerManager):
    ...
    def execute(self, userdata):
        ...
        self.target_distance = error_dis - 0.1 # 少10cm左右
...
class GlobalPickupTask_4(smach.State, SerManager):
    ...
    def execute(self, userdata):
        ...
        dis_line = 1.5
        # print("*"*8)
        delivery_point4 = self.calculate_reverse_point(delivery_point3, self.current_yaw_reverse_distance-cotton_4_num*dis_line)
        # print("-"*8)

        delivery_point5 = self.calculate_reverse_point(delivery_point4, 4)
        
        save_point_to_txt("delivery_point1", delivery_point4)
        self.new_point_reindex.append(delivery_point5)
        ...
        rospy.loginfo("\033[32m"+"下发目标点位"+"\033[0m")
        goal_point, goal_quaternion = delivery_point4
        goal_pose = Point(goal_point[0], goal_point[1], goal_point[2])
        goal_orientation = Quaternion(goal_quaternion[0], goal_quaternion[1], goal_quaternion[2], goal_quaternion[3])
        self.send_target_pose(goal_pose, goal_orientation)
        rospy.loginfo("\033[32m"+"目标点位发布成功"+"\033[0m")
        ...
        while position_ == 0 and not self.brake_enable:
            print("*"*8)
            print("cotton_4_num: ", cotton_4_num)
            print("-"*8)
            # 空的路径
            path_msg = Path()
            path_msg.header.frame_id = 'map'
...
class GlobalPickupTask_3(smach.State, SerManager):
    ...
    def execute(self, userdata):
        ...
        pose_point = ((-25.6370,-12.9885,0.0000),(0.0000,0.0000,0.7086,0.7056))
        reversed_pose_point = self.calculate_reverse_point(pose_point, 3.0)
        ...
        front_point_observe = self.calculate_reverse_point(point_observe, 4)
        
        self.new_point_reindex.append(reversed_pose_point)
        self.new_point_reindex.append(pose_point)
...
class GlobalPickupTask_2(smach.State, SerManager):
    ...
    def execute(self, userdata):
        ...
        self.new_point_reindex.append(front_point_observe)
        self.new_point_reindex.append(point_observe)
        reversed_goal_yaw_point = self.calculate_reverse_point(point_observe, 1.5) # 后移 要求检测距离为2.5m时。
...
class AdjustHeading(smach.State, SerManager):
    ...
    def execute(self, userdata):
        ...
        elif self.delivery_point=="delivery_point":
            self.goal = delivery_point
...
class GlobalMoveTask_CloseLoop(smach.State, SerManager):
    ...
    def execute(self, userdata):
        ...
        print("self.target_distance:", self.target_distance)
        # 前进
        if (self.target_distance>0):
...
def create_state_machine():
    ...
    # 直接判断放货： 当前货架的放货方向
    if not use_cloud:
        delivery_point = rospy.get_param('delivery_point', ((-5.0265,-24.9731,0.0000),(0.0000,0.0000,-0.7121,0.7020)))
    ...
    with sm:
        ...
        # smach.StateMachine.add('GLOBAL_MO', GlobalMoveTask_CloseLoop(target_distance=-1.0, speed=0.3),transitions={'finish': 'FINISH'})
        ...
        with sub_sm:
            ...
            with stack_and_move_sub_sm:
                ...
                with stack_sub:
                    if not use_sim:
                        smach.StateMachine.add('HEIGHT_STACK', SetHeight(two_cargos_height, updown_mode=1),transitions={'finish': 'WIDTH_OPEN_FIRST'} )
                        # 叠放张开
                        smach.StateMachine.add('WIDTH_OPEN_FIRST', SetWidth(236), transitions={'finish': 'stack_finish'})
                    else:
                ...
                smach.StateMachine.add('STACK_SECTION', stack_sub, transitions={'stack_finish': 'MOVE_BACK_DOWN'})

                # 倒车 边倒车边下降
                smach.StateMachine.add('MOVE_BACK_DOWN', GlobalMoveTask_OpenLoop(target_distance=(-0.1), Throttle=0.2), transitions={'finish': 'HEIGHT_LOWER_FINAL'}, )
                if not use_sim:
            ...
            if not use_sim:
                ...
                # 调整航向
                smach.StateMachine.add('ADJUST_HEADING', AdjustHeading(goal='observer_point', steering_angle=adjust_steering_angle_sim, changeYawangle=-90, gear=3, throttle=0.4, control_com=0),transitions={'finish': 'finish'})
        ...
        # 标定点位
        smach.StateMachine.add('LidarObstacleState', LidarObstacle(lidar_obs=1), transitions={'finish': 'GLOBAL_PICKOFF'})
        # 新增放货任务
        smach.StateMachine.add('GLOBAL_PICKOFF', GlobalPickupTask_4(control_task=1, limited_speed=0.8), transitions={'finish': 'AdjustHeading1'})
        ...
        # 先确定放货位置，然后计算外部道路的转向点位置
        smach.StateMachine.add('AdjustHeading1', AdjustHeading(goal='delivery_point', steering_angle=adjust_steering_angle, changeYawangle=0, gear=3, throttle=0.3, control_com=6),transitions={'finish': 'GlobalDelivery'})
        # 然后
        smach.StateMachine.add('GlobalDelivery', GlobalDelivery(goal='delivery_point', Throttle=0.2),transitions={'finish':'HeightRaiseFinal1'})
        ...
        parallel_container5 = smach.Concurrence(outcomes=['all_finished'],
                                        default_outcome='all_finished',
                                        outcome_map={'all_finished': {
                                            'SET_WIDTH_INIT': 'finish',
                                            'GLOBAL_PICKUP': 'finish',
                                            'SET_HEIGHT_INIT': 'finish'}},
                                        input_keys=['conveyer_belt_point', 'observer_point', 'delivery_point'], 
                                        output_keys=['conveyer_belt_point', 'observer_point', 'delivery_point'])
        with parallel_container5:
                if not use_sim:
                    smach.Concurrence.add('SET_WIDTH_INIT', SetWidth(tar_width=25))
                else:
                    smach.Concurrence.add('SET_WIDTH_INIT', SetWidthSim(tar_width=clamping_width_sim))
                smach.Concurrence.add('GLOBAL_PICKUP', GlobalPickupTask_3(control_task=1, limited_speed=0.8))
                if not use_sim:
                    smach.Concurrence.add('SET_HEIGHT_INIT', SetHeight(tar_height=200, updown_mode=1))
                else:
                    smach.Concurrence.add('SET_HEIGHT_INIT', SetHeightSim(tar_height=single_cargo_height_sim))
        smach.StateMachine.add('GLOBAL_PICKUP_4', parallel_container5,transitions={'all_finished': 'SUB_STATE_MACHINE1'})

        smach.StateMachine.add('SUB_STATE_MACHINE1', sub_sm, transitions={'finish': 'LidarObstacleState1'})
        smach.StateMachine.add('LidarObstacleState1', LidarObstacle(lidar_obs=1), transitions={'finish': 'GlobalDelivery1'})
        # smach.StateMachine.add('GlobalDelivery1', GlobalDelivery(goal='delivery_point', Throttle=0.2),transitions={'finish':'AdjustHeading2'})
        smach.StateMachine.add('GlobalDelivery1', GlobalPickupTask_4(control_task=1, limited_speed=0.8), transitions={'finish': 'AdjustHeading2'})
        ...
        smach.StateMachine.add('AdjustHeading2', AdjustHeading(goal='delivery_point', steering_angle=adjust_steering_angle, changeYawangle=0, gear=3, throttle=0.3, control_com=6),transitions={'finish': 'HeightRaiseFinal3'})

        if not use_sim:
            smach.StateMachine.add('HeightRaiseFinal3', SetHeight(single_cargo_height_half, updown_mode=2), transitions={'finish': 'WidthClose2'})
            smach.StateMachine.add('WidthClose2', SetWidth(270), transitions={'finish': 'HeightRaiseFinal4'})
            ...
            smach.StateMachine.add('HeightRaiseFinal4', SetHeightSim(single_cargo_height_sim), transitions={'finish': 'ADJUST_DESIGN_HEADING2'})
