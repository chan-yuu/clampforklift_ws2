...
class GlobalPickupTask_3(smach.State, SerManager):
    def __init__(self,limited_speed=None):
        smach.State.__init__(self, outcomes=['finish'], io_keys=['conveyer_belt_point'])
        SerManager.__init__(self)
...
def create_state_machine():
    ...
    with sm:
        ...
        with pickup_sub_sm:
            ...
            if not use_sim:
                # 取一个包
                smach.StateMachine.add('SET_WIDTH_PICKUP', SetWidth(clamping_width), transitions={'finish': 'HEIGHT_RAISE'})
                # 升高，准备后退
                smach.StateMachine.add('HEIGHT_RAISE', SetHeight(height_between_two_cargos+5, updown_mode=2),transitions={'finish': 'pickup_sub_finish'})
            else:
                # 取一个包
                smach.StateMachine.add('SET_WIDTH_PICKUP', SetWidthSim(clamping_width_sim), transitions={'finish': 'HEIGHT_RAISE'})
                # 升高，准备后退
                smach.StateMachine.add('HEIGHT_RAISE', SetHeightSim(height_between_two_cargos),transitions={'finish': 'pickup_sub_finish'})
        ...
        with stack_and_move_sub_sm:
            ...
            with stack_sub:
                if not use_sim:
                    smach.StateMachine.add('HEIGHT_STACK', SetHeight(two_cargos_height, updown_mode=2),transitions={'finish': 'WIDTH_OPEN'})
                    smach.StateMachine.add('WIDTH_OPEN', SetWidth(open_width_-40), transitions={'finish': 'stack_finish'})
                else:
                    smach.StateMachine.add('HEIGHT_STACK', SetHeightSim(single_cargo_height_sim),transitions={'finish': 'WIDTH_OPEN'})
                    smach.StateMachine.add('WIDTH_OPEN', SetWidthSim(open_width_sim), transitions={'finish': 'stack_finish'})
        ...
        if not use_sim:
            # 调整航向
            smach.StateMachine.add('ADJUST_HEADING', AdjustHeading(goal='observer_point', steering_angle=adjust_steering_angle, changeYawangle=-90, gear=3, throttle=0.4, control_com=6),
                                    # 都是角度制
            ...
            smach.StateMachine.add('ADJUST_HEADING', AdjustHeading(goal='observer_point', steering_angle=adjust_steering_angle_sim, changeYawangle=-90, gear=3, throttle=0.4, control_com=0),
                                    # 都是角度制
                                    transitions={'finish': 'GLOBAL_PICKUP_FINAL'})
            smach.StateMachine.add('ADJUST_DESIGN_HEADING', AdjustHeading_Design_Angle(angle=90,steering_angle=adjust_steering_angle_sim, gear=3, throttle=0.4, control_com=0),
                                    # 都是角度制
                                    transitions={'finish': 'GLOBAL_PICKUP_FINAL'})
