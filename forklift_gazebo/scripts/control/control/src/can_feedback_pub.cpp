...
bool CanFeedBack::Init_CAN(ros::NodeHandle nh_)
{
    ...
    gear_state_pub_ = nh_.advertise<std_msgs::Int8>("/gear_state", 10);
    throttle_state_pub_ = nh_.advertise<std_msgs::Float64>("/throttle_state", 10);
    brake_state_pub_ = nh_.advertise<std_msgs::Float64>("/brake_state", 10);
    steering_state_pub_ = nh_.advertise<std_msgs::Float64>("/steering_state", 10);
    // 夹抱动作相关
    // 横移位置
    lateral_state_pub_ = nh_.advertise<std_msgs::Float64>("/lateral_state", 10);
    // 夹抱位置
    clamp_state_pub_ = nh_.advertise<std_msgs::Float64>("clamp_state", 10);
    // 升降
    updown_state_pub_ = nh_.advertise<std_msgs::Float64>("updown_state", 10);
    // 俯仰
    fy_state_pub_ = nh_.advertise<std_msgs::Float64>("fy_state", 10);

    ifname = nh_.param<std::string>("/can_feedback/ifname", "can1");
    loopRate = nh_.param("/can_feedback/loopRate", 100);
    ros::Rate loop_rate(loopRate); // 调整发布频率的接口
...
CanMessage206 CanFeedBack::parseCanMessage206(const struct can_frame &frame)  //对比叉车改了msg
{
    ...
    msg.lateral_movement_left = static_cast<double>(lateral_movement_left);   //横移左侧可移动的距离 0-1000 ，精度0.1mm

    int16_t lateral_movement_right = ((uint8_t)frame.data[7] << 8) |frame.data[6];
    msg.lateral_movement_right = static_cast<double>(lateral_movement_right);   //横移右侧可移动的距离  0-1000 ，精度0.1mm

    int16_t lateral_movement_position = ((uint8_t)frame.data[3] << 8) |frame.data[2]; ////当前横移位置 有效范围：-1000-1000 ，精度0.1mm，Unsignal类型    
...
void CanFeedBack::DecodeCanData()
{
    ...
    if (read(s, &frame, sizeof(struct can_frame)) > 0)
    {
        ...
        if (frame.can_id == 0x201)
        {
            ...
            // std::cout<<"angle now: "<<steering_msg.data*180/M_PI<<std::endl;
            twist_msg.header.stamp = ros::Time::now();
            twist_msg.header.frame_id = "front_front_axle";
