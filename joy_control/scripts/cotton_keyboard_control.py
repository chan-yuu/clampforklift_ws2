...
LastEditTime: 2024-09-02 02:53:32
FilePath: /undefined/home/cyun/forklift_sim_ws/src/joy_control/scripts/cotton_keyboard_control.py
Description: 用于夹抱车的完整cmd控制
...
def draw_interface():
    ...
    fork_height = int(fork_position / 2.0 * max_fork_height) + 45 # 45让它从0开始增加
    if fork_height >= 0:
    ...
    pygame.draw.rect(screen, (0, 0, 0), fork_label_rect.inflate(10, 10), 1)  # 蓝边
    ...
    pygame.draw.rect(screen, (0, 0, 0), speed_label_rect.inflate(10, 10), 1)  # 蓝边
    ...
    pygame.draw.rect(screen, (0, 0, 0), turn_label_rect.inflate(10, 10), 1)  # 蓝边
...
while running and not rospy.is_shutdown():
    ...
    if fork_updated:
        ...
        if key_status[K_DOWN]:
            left_vertical_target -= 0.05
            right_vertical_target -= 0.05
        if key_status[K_RIGHT]:
            left_horizontal_target += 0.05
            right_horizontal_target -= 0.05
        if key_status[K_LEFT]:
            left_horizontal_target -= 0.05
            right_horizontal_target += 0.05
    ...
    print("Key Status:", left_horizontal_target, right_horizontal_target)

    fork_position = left_vertical_target
